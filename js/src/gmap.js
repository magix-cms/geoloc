class GoogleMap {
						...
						let flags = [
							{
								...
								icon: {
									url: "/"+GM.lang+"/geoloc/?marker=grey&dotless=true",
							...
							},
							{
								...
								icon: {
									url: "/"+GM.lang+"/geoloc/?marker=main&dotless=true",
									labelOrigin: {x:14, y:13}
								},
								label: 'B',
	...
	init() {
		...
		if (GM.markers.length > 0) {
			...
			// debug lat & lng
			//console.log(GM.markers[0].lat);
			//console.log(GM.markers[0].lng);
			...
			GM.markers.forEach((markerDetails,index) => {
				...
				let markerOptions = {
					map: GM.map.instance,
					position: point,
					icon: "/"+GM.lang+"/geoloc/?marker=main",
				};
