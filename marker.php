...
# -- END LICENSE BLOCK -----------------------------------
/**
 * Class plugins_geoloc_marker
 *
 * geoloc marker class allows you to create a marker based on a color
...
class plugins_geoloc_marker {
	...
	/**
	 * plugins_geoloc_marker constructor.
	 * @param string $hex
	...
	public function createMarker(string $name = 'main') {
		...
		file_put_contents(component_core_system::basePath().'/plugins/geoloc/markers/'.$name.'-dotless.svg', $this->template->fetch('marker.tpl'));

		$this->setDotColor();

		file_put_contents(component_core_system::basePath().'/plugins/geoloc/markers/'.$name.'.svg', $this->template->fetch('marker.tpl'));
	}
}